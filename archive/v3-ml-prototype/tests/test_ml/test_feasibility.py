...
from factorforge.ml.metrics import calculate_gc
...
def test_impossible_gc_target_flagged_impossible() -> None:
    result = analyze_feasibility("MK", WEIGHTS, target_cai=0.1, target_gc_low=90, target_gc_high=100)
...
def test_max_cai_without_gc_is_at_least_strict_gc_max() -> None:
    result = analyze_feasibility("MAK", WEIGHTS, target_cai=0.1, target_gc_low=40, target_gc_high=50)
...
def test_local_gc_reporting_for_best_candidate() -> None:
    result = analyze_feasibility("MAK", WEIGHTS, target_cai=0.1, target_gc_low=30, target_gc_high=70)
