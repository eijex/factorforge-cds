...
def _check_codon_concentration(sequence: str, context: Dict[str, Any]) -> Dict[str, Any]:
    """Checks if any amino acid has >80% usage of a single codon (concentration)."""
    STANDARD_CODE = {
    ...
    if len(seq) % 3 != 0:
        return {"status": "ok"} # Skip if not full CDS
    ...
    for aa, total in aa_counts.items():
        ...
        for c, a in STANDARD_CODE.items():
            if a == aa:
                share = c_counts.get(c, 0) / total
                if share > max_share:
                    max_share = share
        if max_share >= 0.80:
            concentrated_aas.append(f"{aa} (Max share: {max_share:.0%})")
            
    if concentrated_aas:
        return {
            "status": "finding",
            "message": f"High synonymous codon concentration detected for: {', '.join(concentrated_aas)}"
        }
    return {"status": "ok"}
