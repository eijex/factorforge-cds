#!/usr/bin/env python3
"""
Pretrain a BART Transformer for codon optimization.
"""
...
from transformers import BartConfig, BartForConditionalGeneration, Trainer, TrainingArguments
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Pretrain BART for codon optimization.")
...
def main() -> int:
    ...
    steps = resolve_value(args.steps, train_cfg.get("steps", 200000))
    ...
    mask_percent = mask_cfg.get("mask_percent", 0.3)
    random_mask = bool(mask_cfg.get("random_mask", True))
    ...
    if model_cfg.get("vocab_size") and model_cfg["vocab_size"] != len(tokenizer.token_to_id):
        logging.warning(
            "Config vocab_size %s != tokenizer vocab_size %s; using tokenizer size.",
